...
const API_URL = "http://localhost:5000/patients"; // Cambia esto si tu backend está en otro puerto
...
const Patients = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      await fetch(`${API_URL}/${selectedPatient.id}`, {
        method: "PUT",
  ...
  const handleSaveNewPatient = async () => {
    if (!selectedPatient || selectedPatient.age < 5 || !selectedPatient.name.trim() || !selectedPatient.lastName.trim() || !selectedPatient.phone.trim()) {
      toast.error("Todos los campos son obligatorios.");
      return;
    }
